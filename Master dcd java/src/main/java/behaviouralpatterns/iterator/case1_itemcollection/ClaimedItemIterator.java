 ...
 *
 * Traverses a list of Items, returning only those whose status is "Claimed".
 ...
 */
public class ClaimedItemIterator implements ItemIterator {
     ...
     */
    public ClaimedItemIterator(List<Item> items) {
        this.items = items;
        this.position = 0;
        advance();
    }
    ...
    private void advance() {
        ...
        while (position < items.size()) {
            ...
            if (candidate.getStatus() != null &&
                candidate.getStatus().equalsIgnoreCase("Claimed")) {
                next = candidate;
                break;
            }
        }
