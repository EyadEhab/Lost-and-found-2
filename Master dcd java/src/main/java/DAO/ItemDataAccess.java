...
public class ItemDataAccess {
     ...
     * Write/read as UTF-8 bytes so JDBC types match the column without implicit
     * nvarchar conversion.
     */
    private static void setPhotoPathColumn(PreparedStatement stmt, int index, String path) throws SQLException {
        if (path == null || path.isEmpty()) {
            ...
            // tagging this parameter as nvarchar, which would cause an implicit-conversion
            // error.
        ...
        try (Connection conn = DBConnection.getInstance().getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    item.setItemType(
                            factory.ItemTypeFactory.getItemType(rs.getString("Category"), rs.getString("Status")));
    ...
    public List<Item> getAllItems() {
        ...
        // Updated to use UploadedByUserID
        String sql = "SELECT ItemID, Title, Description, Category, Location, Photo, DateUploaded, Status, UploadedByUserID FROM FOUND_ITEM ORDER BY ItemID DESC";
        ...
        try (Connection conn = DBConnection.getInstance().getConnection();
            ...
            while (rs.next()) {
                ...
                // Mapped the exact column name from your database!
                item.setOfficerID(rs.getInt("UploadedByUserID"));
    ...
    public int saveItem(Item item) {
        ...
        try (Connection conn = DBConnection.getInstance().getConnection();
            ...
            if (rowsAffected > 0) {
                ...
                // Some SQL Server JDBC setups return no rows from getGeneratedKeys; use session
                // scope
    ...
    // Add this right below updateItemRecord
    public void updateItemStatus(int itemID, String newStatus) {
        String sql = "UPDATE FOUND_ITEM SET Status = ? WHERE ItemID = ?";
    ...
    public List<Item> searchByKeywordsAndCategory(String keywords, String category) {
        ...
        // Available to search: not yet collected (aligns with DB CHECK + DBConnection
        // stats)
